...
The largest net carbon losses were in found in the mountain regions (Sierra Nevada, Klamath and Cascades ecoregions) of California and Oregon and were largely driven by stand-replacing wildifres (California) and forest harvest (Oregon). While remaining a net sink of carbon, the southeast experienced declines to to high rates of harvest of plantation forests and urbanization. However, the high rates of productivity in the region, along with afforestation, offset carbon losses resulting in a modest net sink. Texas was the state with the largest net sink, largely the result of forest ageing and low rates of stand-replacing disturbances.
...
Carbon losses from LULC and disturbances reduced the annual forest carbon sink by `r lulc_losses` MMT C yr^-1^ on average. The cumulative loss of carbon sequestration was primarily from the transfer of carbon to harvest wood products which averaged `r flows_harvest_prim_mean` MMT C yr^-1^ while carbon losses from combusion - primarily due to wildfire - averaged `r flows_emis_prim_mean` MMT C yr^-1^ (Table \@ref(tab:lulcfluxtable)). Carbon losses from fire were highly variable from year-to-year. However, the three highest fire emission years all occurred since 2017. In 2020, emissions from fire, including the record setting megafires in California and Oregon, were estimated to be `r flows_emis_prim_2020` MMT C (`r flows_emis_prim_2020*3.67` MMT CO~2~), more then three times the historical average. In addition to ecosystem carbon removals, LULC and disturbances resulted in the transfer of an additional `r flows_mort_prim_mean` MMT C yr^-1^ from live to DOM pools which will contribute to future years reductions in the carbon sink. This future flux could be accelerated by continued increases in the turnover rate of DOM resulting from continued climate warming. 
...
We estimate U.S. forested ecosystems stored `r stocks_tec_2001` MMT C in 2001 with live carbon accounting for `r stocks_live_2001_pct`% (`r stocks_live_2001` MMT C) and DOM pools accounting for `r stocks_dom_2001_pct`% (`r stocks_dom_2001` MMT C). By 2020, the total carbon stored in live carbon pools increased to `r stocks_live_2020` MMT C while DOM pools remained relatively stable. However, the net change in DOM masked dynamics within DOM pools. For example, carbon stored in standing dead snags increased by nearly 250 MMT C while soil pools declined by 420 MMT C (Table \@ref(tab:stocktable)).  

Changes in LULC were an important driver of carbon stock change. When the effects of LULC were included, increases in live carbon were modest (`r stocks_live_change_prim` MMT C) relative to simulations without LULC and disturbances (`r stocks_live_change_clim` MMT C). Furthermore, changes in LULC and disturbance resulted in increases in litter and deadwood relative to simulations without LULC while soil carbon losses were larger. 
...
kable(stocks_table_merge, col.names = c("Stock", "2001", "2020", "Change"),
      digits = 1,
      booktabs = T,
      escape = F,
      caption="Carbon stock estimates for forest ecosystems of the conterminous U.S.. Units are in MMT C.") %>%
  kable_paper(lightable_options = c("striped"), font_size=12, full_width = F) %>%
...
Talk about species level changes here...
