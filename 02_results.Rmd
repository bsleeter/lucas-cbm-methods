...
The largest net carbon losses were in found in the mountain regions (Sierra Nevada, Klamath and Cascades ecoregions) of California and Oregon and were largely driven by stand-replacing wildifres and forest harvest. While remaining a net sink of carbon, the southeast experienced declines due to high rates of harvest of plantation forests and urbanization. However, high rates of productivity in the region, along with afforestation, offset carbon losses resulting in a modest net sink. Texas was the state with the largest net sink, largely the result of forest ageing and low rates of stand-replacing disturbances.
...
Carbon losses from LULC and disturbances reduced the annual forest carbon sink by `r lulc_losses` MMT C yr^-1^ on average. The cumulative loss of carbon sequestration was primarily from the transfer of carbon to harvest wood products which averaged `r flows_harvest_prim_mean` MMT C yr^-1^ while carbon losses from combustion averaged `r flows_emis_prim_mean` MMT C yr^-1^ (Table \@ref(tab:lulcfluxtable)). Carbon losses from fire were highly variable from year to year. However, the three highest fire emission years all occurred since 2017. In 2020, emissions from fire, including the record setting megafires in California and Oregon, were estimated to be `r flows_emis_prim_2020` MMT C (`r flows_emis_prim_2020*3.67` MMT CO~2~), more then three times the historical average. In addition to ecosystem carbon removals, LULC and disturbances resulted in the transfer of an additional `r flows_mort_prim_mean` MMT C yr^-1^ from live to DOM pools which will contribute to future years reductions in the carbon sink. This future flux could be accelerated by continued increases in the turnover rate of DOM resulting from continued climate warming. 
...
For the western U.S., we projected future carbon storage and flux out to 2050 under two climate scenarios. We used the CanESM2 and HADGEM2-ES365 downscaled climate futures for RCP 4.5 to project changes in carbon storage and flux for U.S. forested ecosystems. Downscaled climate data were from the Multivariate Adaptive Constructed Analogs (MACA) (@abatzoglou2012comparison). Future LULC and disturbances were sampled, with replacement, from historical rates of change following methods described in @sleeter2019effects. 
...
flux_proj = read_csv("data/results/flows_west.csv") %>%
  ...
  filter(FlowGroupID %in% c("Net Biome Productivity")) %>%
  group_by(ScenarioName, FlowGroupID) %>%
  summarise(Mean=mean(Amount)/1000000)
flux_proj
...
stock_proj = read_csv("data/results/stocks_west.csv") %>%
  ...
  # filter(StockGroupID %in% c("Biomass: Aboveground", "Biomass: Belowground", "DOM: Litter", "DOM: Deadwood", "DOM: Soil",
  #                            "Biomass: Total", "DOM: Total", "Total Ecosystem")) %>%
  # mutate(StockGroupID = factor(StockGroupID, levels=c("Biomass: Aboveground", "Biomass: Belowground", "DOM: Litter", "DOM: Deadwood", "DOM: Soil",
  #                            "Biomass: Total", "DOM: Total", "Total Ecosystem")))

p1 = ggplot(stock_proj, aes(x=Timestep, y=Amount/1000000, color=ScenarioName)) +
  geom_line() +
  #geom_point() +
  scale_color_manual(values=c("Historical"="black", "CanESM2"="forestgreen", "HadGEM2-ES365"="red3")) +
  facet_wrap(~StockGroupID, scales="free_y", ncol=1) +
  theme_custom() +
