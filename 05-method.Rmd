...
statemap4k = aggregate(statemap, fact=4, fun="modal", na.rm=T)
statemap_df = as.data.frame(statemap4k, xy=T) %>%
...
ggplot() +
  ...
  theme_ipsum_rc() +
  labs(fill="State Class", x="Longitude", y="Latitude") +
...
```{r, agemap, cache=T, echo=F, warning=F, message=F, fig.width=8, fig.cap="State class map developed by merging the 2001 National Land Cover Database and U.S. Forest Service forest type-groups maps."}
agemap = raster("data/raster/ic-imputed-age.tif")
agemap4k = aggregate(agemap, fact=4, fun="modal", na.rm=T)
agemap_df = as.data.frame(agemap4k, xy=T) %>%
