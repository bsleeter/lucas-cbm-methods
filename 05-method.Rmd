...
We also applied the same equations to gridMet climate normals for the period 1980-2010. The annual NPP multiplier for each cell was estimated as:

$$ NPP_{anom} = \frac{NPP^t} {\mu{NPP}^{1980-2010}}   $$

```{r, growthmult, echo=F, warning=F, messge=F, fig.width=8, fig.height=8, fig.cap="Spatial flow multipliers used to modify annual NPP in the LUCAS model."}
...
ggplot() +
  ...
  scale_fill_distiller(palette="Spectral", type="div", direction=1, guide="colorbar") +
  coord_sf(crs = "+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +units=m +no_defs",
           xlim=c(min(multgrowth_df$x), max(multgrowth_df$x)),
           ylim=c(min(multgrowth_df$y), max(multgrowth_df$y))) +
  theme_bw() +
  labs(x=NULL, y=NULL, fill="Multiplier") +
  theme(text=element_text(family="Roboto Condensed"),
        legend.position = "bottom",
        legend.text.align = 0,
        legend.text = element_text(size = 7, hjust = 0, color = "#4e4d47"),
